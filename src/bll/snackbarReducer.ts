...
type SnackbarMessageTextType = 'error' | 'info' | 'warning';
...
export const snackbarReducer = (state: SnackBarStateType = snackbarInitState, action: ActionsType): SnackBarStateType => {
    switch (action.type) {
        case 'ADD_SNACKBAR_ERROR_MESSAGE':
            return {...state, snackbarMessages: [{type: 'error', id: v1(), ...action.payload}, ...state.snackbarMessages]};
        case 'ADD_SNACKBAR_WARNING_MESSAGE':
            return {...state, snackbarMessages: [{type: 'warning', id: v1(), ...action.payload}, ...state.snackbarMessages]};
        case 'ADD_SNACKBAR_INFO_MESSAGE':
            return {...state, snackbarMessages: [{type: 'info', id: v1(), ...action.payload}, ...state.snackbarMessages]};
